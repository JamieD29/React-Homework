...
export default class Loop extends Component {
  ...
  renderMovies = () => {
    const htmlArr = this.movies.map((item) => {
      return (
        <div className="col-3 mt-4" key={item.maPhim}>
          <div className="card">
            <img src={item.hinhAnh} className="card-img-top w-100" alt="..." />
            <div className="card-body">
              <h5 className="card-title">{item.tenPhim}</h5>
              <h6 className="card-text">{item.ngayKhoiChieu}</h6>
              <p className="card-text">{item.moTa.substring(0,100)}</p>
